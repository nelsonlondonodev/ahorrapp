...
const TransactionItem = memo(function TransactionItem({ transaction, onEdit, onDelete }) {
    ...
    return (
        <li className="flex items-center justify-between p-4 bg-slate-200 dark:bg-slate-800 rounded-lg mb-3">
            <div className="flex items-center space-x-4">
                 <div className={`p-2 rounded-full ${isExpense ? 'bg-red-500/10' : 'bg-green-500/10'}`}>
                    {isExpense ? <ArrowDownIcon /> : <ArrowUpIcon />}
                </div>
                <div>
                    <p className="text-slate-800 dark:text-white font-semibold">{description}</p>
                    <p className="text-slate-500 dark:text-slate-400 text-sm">{category} - {formattedDate}</p>
                </div>
            </div>
            <div className="flex items-center space-x-2">
                ...
                <button onClick={() => onEdit(transaction)} className="text-slate-500 hover:text-slate-800 dark:hover:text-white p-2 rounded-full hover:bg-slate-300 dark:hover:bg-slate-700">
                    <EditIcon />
                </button>
                <button onClick={() => onDelete(transaction.id)} className="text-slate-500 hover:text-slate-800 dark:hover:text-white p-2 rounded-full hover:bg-slate-300 dark:hover:bg-slate-700">
                    <TrashIcon />
                </button>
            </div>
        </li>
